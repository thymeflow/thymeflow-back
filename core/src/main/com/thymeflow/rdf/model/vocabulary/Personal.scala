...
object Personal {
  ...
  val SAME_AS_TEMP: IRI = SimpleValueFactory.getInstance().createIRI(NAMESPACE, "sameAsTemp")
  val SAME_AS_TEMP_SINGLE: IRI = SimpleValueFactory.getInstance().createIRI(NAMESPACE, "sameAsTempSingle")
  val SAME_AS_TEMP_AMBIGUOUS: IRI = SimpleValueFactory.getInstance().createIRI(NAMESPACE, "sameAsTempAmbiguous")
