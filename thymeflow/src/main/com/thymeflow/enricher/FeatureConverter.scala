...
class FeatureConverter(valueFactory: ValueFactory) {
  ...
  def convert(feature: Feature, statements: StatementSet): Resource = {
    ...
    statements.add(placeResource, RDF.TYPE, SchemaOrg.PLACE)
    feature.name.foreach(name =>
      statements.add(placeResource, SchemaOrg.NAME, valueFactory.createLiteral(name))
    )
    statements.add(placeResource, SchemaOrg.ADDRESS, postalAddressConverter.convert(feature.address, statements, null))
    statements.add(placeResource, SchemaOrg.GEO, geoCoordinatesConverter.convert(feature.point.longitude, feature.point.latitude, None, None, statements))
